...
def start(**kwargs):
    if kwargs['login_user'] and kwargs['login_password'] and kwargs['redis_url']:
        server = OutboundSSHServer('0.0.0.0', port=9000, login_user=kwargs['login_user'], login_password=kwargs['login_password'], redis_url=kwargs['redis_url'])
...
if __name__ == '__main__':
    ...
    if len(argv) == 4:
        try:
            start(login_user=argv[1], login_password=argv[2], redis_url=argv[3])
